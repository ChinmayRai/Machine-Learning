...
y1=zeros(m,size(Theta2,1));     q=1:num_labels;

for i=1:m
  y1(i,:)=(y(i)==q);
end;

a=(-y1).*log(a3)+(y1-1).*log(1-a3);                 c=sum(sum(a))/m;

t1=Theta1(1:size(Theta1,1),2:size(Theta1,2));       t2=Theta2(1:size(Theta2,1),2:size(Theta2,2));

b=(sum(sum(t1.^2))+sum(sum(t2.^2)))/(2*m);          J=c+lambda*b;

 D_1=zeros(size(Theta1));                           D_2=zeros(size(Theta2));
...
for t=1:m
  ...
  d_3=a_3-(y1(t,:))';
  d_2=(Theta2)'*d_3.*sigmoidGradient([1;z_2]);         d_2=d_2(2:end);
  
 
  D_1=D_1+d_2*(a_1)';                                D_2=D_2+d_3*(a_2)';
end;

Theta1_grad = D_1/m+(lambda/m)*[zeros(size(d_2),1) Theta1(:,2:end)];  Theta2_grad =D_2/m+(lambda/m)*[zeros(size(d_3),1) Theta2(:,2:end)]; 
